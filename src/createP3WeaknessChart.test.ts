import createP3WeaknessChart from "./createP3WeaknessChart";
...
describe("createP3WeaknessChart", () => {
  beforeEach(() => {
  ...
  it("should generate a weakness chart and return an image buffer", async () => {
    ...
    const p3ChartCanvasBuffer =
      await createP3WeaknessChart(weaknesses);

    // Ensure output is a Buffer
    expect(p3ChartCanvasBuffer).toBeInstanceOf(Buffer);

    // Ensure createCanvas was called with expected dimensions
    expect(createCanvas).toHaveBeenCalledWith(2200, 275);
