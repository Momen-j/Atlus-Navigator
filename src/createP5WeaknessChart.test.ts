import createP5WeaknessChart from "./createP5WeaknessChart";
...
describe("createP5WeaknessChart", () => {
  beforeEach(() => {
  ...
  it("should generate a weakness chart and return an image buffer", async () => {
    ...
    const p4ChartCanvasBuffer = await createP5WeaknessChart(weaknesses);
    ...
    expect(createCanvas).toHaveBeenCalledWith(1825, 275);
  ...
  it("should draw images on the canvas for each cell within the weakness chart", async () => {
    ...
    await createP5WeaknessChart(weaknesses);
