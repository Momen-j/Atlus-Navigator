...
async function scrapeData() {
  ...
  await page.goto("https://aqiu384.github.io/megaten-fusion-tool/p5r/shadows", {
  ...
  const data = await page.evaluate(() => {
    ...
    return rows.slice(5).map((row) => {
      ...
      return {
        ...
        gun: columns[6]?.innerText.trim() || "",
        fire: columns[7]?.innerText.trim() || "",
        ice: columns[8]?.innerText.trim() || "",
        elec: columns[9]?.innerText.trim() || "",
        wind: columns[10]?.innerText.trim() || "",
        psychic: columns[11]?.innerText.trim() || "",
        nuke: columns[12]?.innerText.trim() || "",
        light: columns[13]?.innerText.trim() || "",
        dark: columns[14]?.innerText.trim() || "",
        almighty: columns[15]?.innerText.trim() || "",
        drops: columns[16]?.innerText.trim() || "",
        appears: columns[17]?.innerText.trim() || "",
      };
    });
  });
...
async function main() {
  ...
  try {
    ...
    const filePath = path.resolve(__dirname, `P5Rdata-${timestamp}.csv`);
