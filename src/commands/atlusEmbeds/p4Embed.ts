...
export default {
  name: "p4g",
  description: "Provides info on a monster/persona within Persona 4 Golden",
  options: [
    {
      name: "monster-name",
  ...
  callback: async (
    ...
    let dbResult: P4EnemyWeaknesses[];
    ...
    try {
      ...
      dbResult = await fetchP4EnemyWeaknesses(monsterName);
      enemyStats = await fetchP4EnemyStats(monsterName);
    } catch (error: any) {
