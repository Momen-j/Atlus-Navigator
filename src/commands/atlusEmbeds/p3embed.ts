...
export default {
  name: "p3r",
  description:
    "Provides info on a monster/persona within Persona 3 Reload (not including Aigis)",
  options: [
    {
      name: "monster-name",
  ...
  callback: async (
    ...
    try {
      ...
      dbResult = await fetchP3EnemyWeaknesses(monsterName);
      enemyStats = await fetchP3EnemyStats(monsterName);
    } catch (error: any) {
    ...
    if (dbResult.length === 0) {
      return interaction.reply({
        content:
          "❌ This shadow does not exist within the world of Persona 3 Reload.",
